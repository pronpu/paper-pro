...
（3）针对在引入统一入口后仿真高频态势流与控制流共享网关转发链路引发的拥塞瓶颈，提出控制流与态势流分离的双通道传输优化机制。在浏览器侧部署 Service Worker 拦截层，依据直连拓扑视图（DirectTopology）实现流量分流。控制流继续经中心网关处理，而态势流则绕过中心路径，由实例侧 Envoy 执行校验后直接转发，并辅以有限状态机（FSM）策略确保路径异常时的业务连续性。
