...
在导弹等装备仿真领域，分布式仿真计算广泛应用于信号级仿真。随着云计算与虚拟化技术的发展，将半实物仿真系统中的公共计算资源及数字样机部署到私有云或虚拟化平台，已成为分布式仿真系统的重要发展方向。现有分布式仿真系统设计时主要存在以下问题：

第一，现有系统往往采用 TCP/IP、DDS、gRPC 等不同技术实现多子系统协同，未来接入的开源或商业软件还可能采用 HTTP/JSON 等接口形式，导致系统间接口不统一、互操作困难、集成成本较高。

第二，现有系统主要面向单用户、一次一任务的使用模式。在该模式下，一次仿真任务通常独立占用参试物理装置以及计算、网络等资源，任务内部程序只需面向本次仿真协作，无需考虑其他任务干扰。

针对上述问题，本文提出以下解决方案，具体工作如下：

（1）针对不同仿真系统接口形式不一致导致的集成困难，提出以 gRPC 为主要统一接口，对已有仿真系统进行封装屏蔽，并兼容 HTTP/JSON 等新接口。在此基础上，引入统一网关入口，采用 Envoy 完成 gRPC-Web、HTTP/JSON 到内部 gRPC 调用的转换，在保持原有仿真服务内部接口基本不变的前提下，实现多协议统一接入，提高遗留系统集成能力。

（2）仿真任务运行时，模拟导弹、目标及其关键物理与演算部件需要在相同时空内协同推演，因此单个仿真内部的多个程序必须保持有状态协作关系。当系统使用模式进化为多用户同时仿真或单用户同时启动多个仿真时，为避免多个任务间相互干扰，需要将任务与其所属程序实例建立稳定绑定关系，并进行状态管理。而现有 RPC 框架和 Envoy 原生路由机制均无法支持这一需求。针对这一问题，本文构建分布式状态管理机制：在任务创建阶段动态分配专属实例组并维护任务—实例绑定关系；在请求进入系统后依据任务标识精确转发至对应实例；在任务执行过程中通过上下文传播与一致性校验保证任务内通信始终受限于本任务范围，从而避免跨任务访问。进一步提出基于 Envoy 和 Consul 的实现方案，满足多仿真任务并行需求。

（3）为避免 Envoy 网关作为所有仿真服务调用中介带来的性能与时延问题，提出控制流与态势流分离的双通道传输优化机制：低频的控制请求继续经中心网关完成统一治理，高频的态势数据通过部署于参试节点的端侧 Envoy 建立直达路径，并结合异常回退机制保障系统连续运行。

实验结果表明，本文提出的统一接入、分布式状态管理和双通道传输优化机制能够有效支撑典型遗留分布式仿真系统改造，实现多用户、多任务场景下的稳定运行，并具备良好的性能表现。
\begin{keywords}                                        % 中文关键词开始
        分布式仿真 \sep 有状态服务 \sep 异构接口集成 \sep Envoy \sep Consul                       %
...
在云端多任务并发场景下，控制面（Control Plane）需实时维护可分配实例集合、任务与实例的映射关系及其运行期元数据。Consul 作为一种典型的分布式服务发现与配置管理系统，其核心机制涵盖服务注册、健康检查及一致性协议~\cite{HashiCorpConsulDocs,DXJS201611004}。
...
Consul KV 的核心特征在于其基于 Raft 共识算法的存储模型。在控制面中，Consul Server 集群通过选举产生 Leader 节点。所有针对 KV 数据的写操作（Write）均由 Leader 统一接收并记录日志，随后通过 Raft 协议同步至集群内的多个 Follower 节点。只有当多数派节点确认写入成功后，该配置变更才会持久化到存储中。这种 CP 型（一致性与分区容错性）设计保证了在网络分区或节点故障的情况下，系统依然能够提供统一的配置视图，避免了分布式环境下的配置漂移问题。
...
为降低配置更新延迟，Consul KV 引入了基于长轮询（Long Polling）的 Watch 监听机制。在数据面（Data Plane）中，各微服务节点通过本地部署的 Consul Client 代理，对特定的 KV 路径发起异步监听请求。当管理员更新 KV 数据时，Leader 节点触发状态变更并通知关联的 Client 节点。Client 代理随后将更新信号反馈至对应的微服务应用，驱动应用在不重启进程的前提下完成内存变量的热更新。该模式极大地优化了系统的响应速度，有效支撑了分布式环境下对高频次变更的需求。
...
\begin{figure}[htb]
    ...
    \label{fig:4.1}
\end{figure}
图~\ref{fig:4.1} 概括了系统在控制面、边界层与数据面之间的核心交互过程，分为创建绑定、访问路由与释放回收三个阶段。TaskManager 通过服务发现与互斥占用完成实例绑定，并将任务映射写入 Consul KV 形成任务上下文（TaskContext）；Envoy 在运行期依据任务标识查询任务上下文并解析目标实例完成动态转发；仿真实例在入站阶段执行任务标识（TaskId）一致性校验以实现跨任务隔离兜底。
...
对于进入动态集群的有状态业务流量，系统必须在转发前实时解析其物理坐标。本文通过 Envoy 内置的 Lua 脚本，实现了“上下文提取—地址解析—元数据注入”的闭环，完成了从逻辑任务到物理实例的动态映射。如图~\ref{fig:4.6} 所示，该动态寻址机制包含五个核心步骤，形成了一条严密的数据面处理管线：
\begin{figure}[htbp]
    \centering
    % 请确保文件名正确，且文件存在于项目根目录下
...
[1]XX. 基于 eBPF 技术和插件化技术的分布式仿真系统设计方法[P]. 中国: 2024XXX68995.7, 2024-09-03.

[2]XX. 基于用户态文件与内核虚拟机的异构文件系统集成方法[P]. 中国: 2024XXX41033.2, 2024-09-13.
