...
\begin{abstract}                                            % 中文摘要开始
 现代分布式仿真技术已广泛应用于国防科技领域。随着仿真平台向云端部署并引入云原生微服务治理机制，遗留仿真系统在多租户共享与资源池化环境下面临新的挑战：一方面，需要在最小侵入既有仿真内核的前提下扩展接入形态与统一入口；另一方面，多任务并发打破了传统有状态仿真的独占隔离假设，极易引发跨任务的状态污染。在上述背景下，本文设计并实现了一个具备有状态隔离能力的云端分布式仿真系统。具体工作如下：
...
（3） 针对在引入统一入口后高频态势流与控制流共享网关转发链路引发的拥塞瓶颈，提出控制流与态势流分离的双通道传输优化机制。在浏览器侧部署 Service Worker 拦截层，依据动态直连拓扑（DirectTopology）实现流量分流。控制流继续经中心网关处理，而态势流则绕过中心路径，由实例侧 Envoy 执行校验后直接转发，并辅以有限状态机（FSM）策略确保路径异常时的业务连续性。
...
Modern distributed simulation technology has been widely applied in the field of defense science and technology. As simulation platforms transition toward cloud deployment and incorporate cloud-native microservice governance mechanisms, legacy simulation systems face new challenges in multi-tenant shared and resource-pooled environments. On one hand, there is a necessity to expand access forms and unify entries with minimal intrusion into existing simulation kernels. On the other hand, multi-task concurrency breaks the exclusive isolation assumption of traditional stateful simulations, which frequently leads to cross-task state pollution. In this context, this paper designs and implements a cloud-based distributed simulation system with stateful isolation capabilities. The specific contributions are as follows:
...
仿真技术是对现实世界或复杂系统进行建模、分析与推演的重要手段，广泛应用于军事、航天、交通及复杂工程系统等领域~\cite{WILCOX2000201,XTFZ202004011,1011051997}。随着仿真对象规模与场景复杂度提升，仿真系统由单机模式逐步演进为分布式协同形态，以支撑多要素、多模型、多层级的联合推演需求~\cite{RISCOMARTIN2022102539,XTFZ202311001}。
...
\begin{figure}[htb]
    ...
    \caption{云原生微服务治理的控制面与数据面协同关系示意图}
...
如图~\ref{fig:2_grpc_stack} 所示，gRPC 基于 HTTP/2 协议，使用 Protocol Buffers 作为接口定义语言，通过编译 .proto 文件生成服务端和客户端代码。服务契约定义接口和消息格式，生成的代码处理序列化、网络传输，实现跨语言的高效 RPC 通信~\cite{wang1993grpc,pamadi2020comparative,biswas2018accelerating,geng2024rr}。
...
如图 \ref{fig:2_grpc_modes} 所示，gRPC 的交互模型可抽象为四类：一元调用、服务端流、客户端流与双向流。在云端仿真平台中，控制指令通常符合低频、强一致的请求—响应语义，适合采用一元调用；高频态势数据回传则更接近持续输出的流式语义，常见实现为服务端流或双向流。在后续章节中，本文将控制流与态势流视为不同的流量类型，并在入口治理与端侧分流中对其采取差异化路径，以避免二者在同一转发链路上竞争队列资源~\cite{niswar2024performance}。

\begin{figure}[htb]
    \centering
    \includegraphics[width=15cm]{2.4.png}
    \caption{gRPC 四种通信模式与典型适用场景对比图}
...
在云端多任务并发场景下，控制平面（Control Plane）需实时维护可分配实例集合、任务与实例的映射关系及其运行期元数据。Consul 作为一种典型的分布式服务发现与配置管理系统，其核心机制涵盖服务注册、健康检查及一致性协议~\cite{mishra1993consul,DXJS201611004}。

\begin{figure}[htb]
    \centering
    \includegraphics[width=15cm]{2.6.png}
    \caption{Consul 服务注册原理示意图}
...
在微服务架构中，动态路由是实现流量精准调度、灰度发布及高可用性的核心机制~\cite{lim2024opportunities}。基于 Envoy 代理的动态路由本质是根据请求的元数据信息，将流量动态映射至后端目标集群的过程~\cite{palaniappan1992envoy,aldas2023cloud,cvetkovic2025towards}，如图 \ref{fig:2_9_envoy_dynamic_routing} 所示。

\begin{figure}[htb]
    \centering
    \includegraphics[width=15cm]{2.9.png}
    \caption{Envoy 动态路由原理示意图}
...
\begin{figure}[htb]
    ...
    \caption{浏览器侧 Service Worker 拦截层原理示意图}
...
进一步而言，云端仿真系统通常呈现典型的新老技术并存特征：前端侧为了获得良好的人机交互与可视化能力，倾向采用浏览器与现代 Web 技术栈；后端侧则往往承载经过长期验证的 C/C++ 仿真内核与专用通信接口，以保证性能、确定性与稳定性。在该现实约束下，系统演进更常采用增量式与迭代式改造路径，而非一次性重写核心内核；同时，需要通过松耦合适配与桥接机制降低不同协议、不同技术栈之间的集成成本。因此，协议转换与统一入口治理并非单纯的工程便利选择，而是面向遗留仿真系统云端化演进的一种低风险、可持续的架构路径。
...
上述两类需求共同指向同一核心矛盾：外部异构协议生态与内部专用 gRPC 生态之间存在协议与契约失配。理论上，可通过重构后端 C/C++ 服务使其原生支持 HTTP 与 WebSocket 来消除失配，但考虑到仿真内核的复杂性与稳定性要求，该路径将触及关键调用链路并引入新的并发与序列化实现，工程风险高且难以兼顾内部节点间高性能通信需求。基于工程可行性与演进成本综合权衡，本文选择在系统边界引入协议转换与流量治理层，在不修改存量业务代码的前提下实现差异化接入能力，如图~\ref{fig:3_protocol_conversion} 所示。
...
如伪代码所示，线程在遍历候选实例列表时，会尝试调用锁抢占接口。只有当抢占返回成功时，线程才被视为获得了该实例的独占分配权，进而继续执行后续的路由表写入和实例初始化操作。若抢占失败，线程必须立即放弃当前实例，并尝试列表中的下一个候选资源。为了直观验证该机制在极端并发场景下的有效性，如图~\ref{fig:4.3} 我们设计了一个 TaskManager 内部线程级并发抢占的典型案例进行分析。
...
\begin{figure}[htbp]
    ...
    \caption{基于 Envoy 过滤器链的入站请求处理流程图}
    \label{fig:4.5}
\end{figure}
...
\begin{table}[!h]
    \centering
    \caption{grpc\_json\_transcoder 关键配置项说明}
...
\begin{figure}[htbp]
    ...
    \caption{基于 Envoy 与 Consul 的任务级动态路由机制示意图}
    \label{fig:4.6}
\end{figure}
...
对于仿真业务服务：在复杂的推演场景中，同一个任务往往需要并发运行多个同类型的仿真模型（例如多台同构的装甲车辆）。为了区分它们，系统在键名中引入了实例序号（Index），格式定为 task/\{TaskId\}/\{Service\}/\{Index\}。表 4-2 中的 /0 即代表该服务在当前任务中的第一个实例。这种设计不仅能精确找到特定的物理节点，也为后续同类服务的横向扩容留足了空间。
\begin{table}[!h]
    \centering
...
在前文构建的任务级状态隔离架构中，外部请求首先经由 Envoy 中心入口网关进入系统，再依据 TaskId 查询控制面维护的 TaskContext，并被转发至对应任务绑定的专属仿真实例组。该机制解决了外部请求按任务边界到达目标实例的问题，但并未覆盖任务进入数据面后的内部协同过程。对于本文所研究的分布式仿真系统，一个任务通常由多个仿真实例协同完成，实例之间仍需在运行期交换控制信息、状态数据或阶段性结果。因此，在入口侧完成任务级动态路由之后，系统还需进一步解决任务内实例之间的对端发现、运行期寻址以及异常请求阻断问题。

为此，本文设计了由服务注册、任务初始化、本地通信视图构建、逻辑服务标识动态寻址以及入站一致性校验组成的任务内协同通信闭环。其中，服务注册与任务初始化负责将控制面的任务分配结果转化为数据面可直接使用的本地状态；动态寻址与连接复用用于支撑任务内高频协同调用；基于 TaskId 的入站校验则作为最终安全兜底，防止错误请求跨越任务边界进入非目标实例。

\subsection{任务内协同通信需求与问题场景}

图~\ref{fig:4_task_internal_comm_scenario} 给出了静态地址配置模式与任务动态分配模式下任务内协同通信问题场景的对比。在传统静态预部署分布式仿真系统中，实例之间的通信关系通常在部署阶段预先确定，节点依据配置文件中写定的 IP 地址、端口和调用关系建立连接，运行期间通信对象基本保持不变。在这种模式下，实例间协同虽然同样存在，但其寻址依据来源于静态配置，而非运行期动态生成。
\begin{figure}[H]
    \centering
    \includegraphics[width=15cm]{4.8.png}
    \caption{静态地址配置与任务动态分配条件下的任务内协同通信问题场景示意图}
    \label{fig:4_task_internal_comm_scenario}
\end{figure}
本文面向云端多任务并发场景构建的系统不再采用固定实例永久绑定方式，而是在任务创建阶段由控制面依据资源可用性与任务隔离约束，为每个 TaskId 动态分配一组专属仿真实例。此时，同一逻辑角色在不同任务中所对应的物理实例地址不再固定，实例在任务创建之前也无法预知当前任务中需要协同的对端节点地址。若仍沿用静态配置方式，则一方面无法适应任务动态分配带来的端点变化，另一方面在多任务并发条件下容易产生请求误投、跨任务误连与状态串扰问题，从而破坏任务级隔离边界。

因此，在入口侧完成基于 TaskId 的精确路由之后，系统还必须解决任务内部实例“如何获得当前任务中的协同对象信息、如何完成运行期寻址以及如何阻断异常访问”三个问题。

如图~\ref{fig:4_task_internal_comm_scenario} 所示，在静态预部署模式下，各节点依据本地配置即可建立连接；而在任务动态分配模式下，控制面仅在任务创建完成后才能确定当前任务绑定的实例集合及其物理端点，实例自身并不知道本任务中的协同对象信息，因而无法继续依赖静态配置完成任务内通信。由此，系统必须引入面向任务运行期的动态寻址机制，并结合入站一致性校验形成任务内协同通信闭环。


\subsection{服务注册与任务初始化驱动的本地通信视图构建}

为解决任务动态分配后实例无法预知对端端点的问题，本文采用“服务注册 + 任务初始化下发”的两阶段机制构建本地通信视图。其基本思路是：实例在空闲状态下首先向注册中心登记自身可达端点与健康状态，使控制面掌握当前可参与任务分配的候选实例集合；任务创建并完成实例绑定后，控制面再将当前任务的对端实例信息通过 InitTask 定向下发至参与节点，各实例据此在本地生成仅面向当前任务的通信视图。

在服务注册阶段，仿真实例启动后向注册中心登记逻辑服务名称、实例标识、宿主节点地址、监听端口及健康状态等信息，注册中心通过健康检查或心跳续租维护实例可用性，仅将健康实例暴露给控制面参与后续任务分配。这里的服务注册并非用于实例运行期主动广播式发现，而是用于支撑控制面统一维护系统中的健康实例集合。也就是说，服务注册解决的是“控制面知道有哪些实例可被分配”的问题。

在任务初始化阶段，控制面基于任务创建结果生成面向该任务的局部协同关系描述，并通过 InitTask 将其下发至各参与实例。与注册阶段维护系统级实例池不同，初始化阶段关注的是“当前任务具体使用了哪些实例，以及这些实例之间应如何建立通信关系”。因此，InitTask 请求中仅包含与当前任务相关的必要信息，例如 TaskId、当前实例角色、其他参与实例的逻辑服务标识及对应物理端点等。这样，控制面关于任务实例绑定的权威结果便被转化为数据面实例本地可直接使用的运行期状态。

图~\ref{fig:4_local_topology_build} 给出了服务注册与任务初始化驱动的本地通信视图构建过程。
\begin{figure}[htbp]
    \centering
    \includegraphics[width=15cm]{4.9.png}
    \caption{服务注册与任务初始化驱动的本地通信视图构建过程示意图}
    \label{fig:4_local_topology_build}
\end{figure}
如图~\ref{fig:4_local_topology_build} 所示，实例注册与健康维护阶段为控制面提供可分配实例集合，任务绑定与初始化下发阶段则将该集合中的选取结果转化为当前任务的局部通信视图。通过这一过程，控制面上的分配结果被落地为数据面实例可直接使用的本地状态，为后续任务内协同调用提供寻址依据。

实例在初始化完成后维护一份面向当前任务的 LocalTaskContext，用于承载任务绑定关系与通信视图。该上下文至少包含三个核心要素：当前实例绑定的任务标识 bound\_task\_id、实例生命周期状态 instance\_state，以及从逻辑服务标识到物理端点映射的 peer\_view。必要时，还可增加 peer\_view\_version 等辅助字段，用于标识本地视图版本或更新时间。
...
需要指出的是，peer\_view 并非系统全局拓扑的完整副本，而是相对于当前实例的任务局部视图，仅保留当前任务执行所需的对端信息。这样的设计既满足运行期寻址需求，又避免了不必要的信息暴露。其生命周期与任务绑定过程保持一致：在任务初始化成功时写入，在任务运行阶段只读使用，在任务结束或实例释放时统一清空，从而避免旧任务残留状态被错误复用。
...
\begin{algorithm}[htbp]
    ...
    \end{algorithmic}
\end{algorithm}
算法~\ref{alg:4_local_context_init} 表明，本地通信视图构建需满足两个基本约束：其一，实例尚未被其他任务占用；其二，TaskId 写入与对端视图构建在同一初始化过程中完成。这样可以保证运行期任务归属信息与通信寻址依据来源一致，避免二者不一致带来的协同异常。

综上，服务注册解决的是控制面“知道有哪些实例可被分配”的问题，任务初始化解决的是数据面实例“知道当前任务中应与谁协同”的问题；二者共同构成了本地通信视图生成的基础。

\subsection{基于逻辑服务标识的动态寻址与连接复用}

在完成任务初始化后，参与同一任务的各实例已经在本地获得了面向当前任务的通信视图。此时，系统还需进一步解决运行期调用目标如何表达、如何将其解析为真实网络端点，以及如何降低高频协同调用中的重复建连开销。

在静态预部署模式下，实例间调用通常直接依赖固定 IP 地址与监听端口。这种方式实现简单，但其前提是实例部署位置与通信关系在运行期间保持不变。在任务动态分配场景下，同一逻辑角色在不同任务中所对应的实例地址不再固定，若调用方仍直接依赖物理地址，将导致业务逻辑与具体部署结果紧耦合，不利于任务迁移、实例替换和拓扑重建。因此，本文不再将物理端点作为实例间协同调用的直接表达对象，而是引入基于逻辑服务标识的动态寻址方式，将“调用谁”与“对方位于哪里”分离。

所谓逻辑服务标识，是指在任务内部用于描述协同对象职责或角色的抽象标识。调用方在发起任务内 RPC 时，不直接面向 IP 地址与端口编写调用逻辑，而是以逻辑服务标识作为目标，由本地 peer\_view 将其解析为当前任务中的真实物理端点。这样，业务侧调用形式在任务之间保持稳定，而具体端点变化被限制在初始化阶段构建的本地视图内处理，从而使运行期协同调用能够适应实例动态分配带来的地址变化。

图~\ref{fig:4_logical_addressing_reuse} 给出了基于逻辑服务标识的动态寻址与连接复用路径。
\begin{figure}[htbp]
    \centering
    \includegraphics[width=15cm]{4.10.png}
    \caption{基于逻辑服务标识的动态寻址与连接复用路径示意图}
    \label{fig:4_logical_addressing_reuse}
\end{figure}
如图~\ref{fig:4_logical_addressing_reuse} 所示，当某一实例需要向任务内协同对象发起调用时，首先给出目标逻辑服务标识；随后，本地寻址模块从 peer\_view 中查找与该标识对应的物理端点；若查找成功，则进一步获取与该端点关联的 gRPC Channel，并在其上构造 Stub 发起远程调用；若查找失败，则立即返回显式错误。通过这一过程，调用方无需感知对端实例的具体部署细节，而是依赖本地通信视图完成运行期寻址。

在此基础上，本文进一步引入连接复用机制，以降低高频协同场景下重复建连的开销。对于分布式仿真任务而言，实例间调用通常具有频率高、重复性强、目标相对稳定等特点。若每次请求都临时创建新的 gRPC Channel，将引入额外的连接建立、资源分配与尾延迟抖动。因此，本文以目标端点为键维护本地 ChannelPool：当调用方完成端点解析后，优先复用已有 Channel；仅在未命中时新建连接并写入连接池。这样，网络连接管理被从业务逻辑中抽离，转化为统一的端点级缓存机制，使任务内高频协同路径具有更稳定的性能表现。
...
\begin{algorithm}[htbp]
    ...
    \end{algorithmic}
\end{algorithm}
算法~\ref{alg:4_get_stub_dynamic} 表明，运行期协同调用形成了“逻辑标识表达—本地视图解析—端点级连接复用—Stub 发起调用”的稳定路径。通过这一机制，控制面一次下发的任务绑定结果能够在数据面被多次复用，从而避免高频调用过程中持续依赖控制面在线参与。

除对端解析与连接复用外，任务内协同调用还需保证任务上下文能够沿调用链一并传播。如图~\ref{fig:4.11}所示、本文在调用方实例侧引入 gRPC 客户端拦截器，在发起远程调用时自动将当前实例本地 LocalTaskContext 中的 bound\_task\_id 写入请求 metadata 的 X-Task-Id 字段。这样，任务标识的携带过程被收敛到统一的调用链入口，而无需由各业务方法显式传参，从而避免了人工传递 TaskId 带来的侵入性与遗漏风险。

采用客户端拦截器自动注入 TaskId 具有两方面作用。其一，它保证了任务内所有出站 RPC 在协议层面携带一致的任务上下文，使后续接收方能够基于统一字段完成入站判定；其二，它将任务标识传播机制从具体业务逻辑中解耦出来，使“调用谁”和“请求属于哪个任务”分别由动态寻址机制与上下文传播机制负责，增强了调用路径的清晰性与可维护性。由此，任务内协同调用不再只是完成对端解析与连接建立，还能够在跨实例传播过程中持续保持与当前任务绑定关系一致的上下文语义。
\begin{figure}[htbp]
    \centering
    \includegraphics[width=15cm]{4.11.png}
    \caption{基于gRPC客户端拦截器的任务标识自动传播机制}
    \label{fig:4.11}
\end{figure}
需要说明的是，动态寻址与连接复用机制解决的是“如何正确找到并高效连接对端实例”的问题，而不能替代异常场景下的最终隔离防线。即使本地 peer\_view 已经构建完成，运行期仍可能因视图过期、错误复用旧上下文或误路由等原因，使请求携带错误 TaskId 或到达不属于当前任务的实例。因此，在实例接收侧仍需进一步引入基于 TaskId 的入站一致性校验机制。

\subsection{基于任务标识和拦截器的入站一致性校验与安全阻断}

通过前述机制，系统已经具备了任务内实例间正确建立协同路径的能力。然而，对于多任务并发条件下的有状态分布式仿真系统而言，仅能“找到正确对端”仍然不够，系统还必须在请求到达错误实例时能够立即阻断。否则，一旦异常请求进入非本任务实例内部，前文建立的任务级隔离边界仍可能被破坏。

这一约束来源于有状态仿真任务的运行特征。本文中的数据面实例并非无状态服务副本，而是在任务初始化后持有与当前任务绑定的 LocalTaskContext，并在运行过程中承载任务状态演化、事件处理与阶段性结果累积。因此，请求能否进入某一实例处理，不仅取决于接口是否存在、网络是否可达，更取决于该请求是否属于实例当前绑定的任务。若一个本应属于任务 $T_1$ 的请求由于视图过期、上下文污染或误路由等原因被发送到了绑定任务 $T_2$ 的实例，即使该请求在语法层面合法，也不应被视为可接受请求。否则将直接引发状态污染与任务串扰。

基于上述考虑，如图~\ref{fig:4_server_interceptor_check}，本文在实例侧 gRPC 服务入口统一部署 gRPC 服务端拦截器，对所有入站 RPC 请求执行基于 TaskId 的前置一致性校验。
\begin{figure}[htbp]
    \centering
    \includegraphics[width=15cm]{4.12.png}
    \caption{基于 TaskId 的服务端入站校验流程示意图}
    \label{fig:4_server_interceptor_check}
\end{figure}
该机制并不替代前文的动态寻址过程，而是作为其后的兜底防线存在：动态寻址负责尽可能让请求找到正确实例，入站拦截负责在异常情况下保证错误请求不会进入后续仿真处理逻辑。与将校验逻辑分散写入各业务接口相比，采用 gRPC 服务端拦截器能够将 TaskId 判定统一收敛到实例服务入口，实现对所有 RPC 的一致约束，同时避免侵入具体仿真方法内部。

从处理位置上看，gRPC 服务端拦截器位于入站 RPC 请求进入具体处理逻辑之前。请求到达实例后，首先由 gRPC 运行时将 metadata 交由拦截器处理；拦截器从中提取 X-Task-Id，并结合本地 LocalTaskContext 执行逐级判定。只有当请求与当前实例绑定任务关系一致、且实例处于可服务状态时，请求才被继续转交给后续处理流程；否则，拦截器立即返回错误码并记录审计信息。由此，任务边界的最终判定点被固定在实例服务入口，而不是延后到仿真计算逻辑内部。

如图~\ref{fig:4_server_interceptor_check} 所示，入站校验流程可概括为三道连续防线。

（1）第一道防线是 TaskId 完整性校验：若 X-Task-Id 缺失、为空或格式非法，则应直接拒绝，因为系统无法确认该请求的任务归属。

（2）第二道防线是实例生命周期状态校验：即使请求携带合法 TaskId，若实例尚未完成初始化或当前状态不满足处理前提，请求同样不应被继续处理。

（3）第三道防线是入站 TaskId 与本地绑定 TaskId 的一致性校验：若二者不一致，则说明该请求并不属于当前实例所绑定的任务，必须视为跨任务访问并予以阻断。

表~\ref{tab:4_inbound_check_rules} 将入站异常归纳为三类具有明确边界的判定场景，使“何种情况下拒绝请求、拒绝依据是什么、拒绝后记录哪些信息”具备统一语义，也为后续实现与测试提供了可验证规则。
\begin{table}[!h]
    \centering
...
\begin{algorithm}[htbp]
    ...
    \end{algorithmic}
\end{algorithm}
算法~\ref{alg:4_server_interceptor} 表明，服务端拦截器以短路式方式依次执行 TaskId 完整性校验、实例状态校验和任务一致性校验，任一环节失败即立即终止请求处理。这样，任务级隔离规则便不再只是控制面与路由层的静态设计要求，而是在数据面服务入口被真正强制执行。

需要说明的是，gRPC 服务端拦截器并不是任务内协同机制的起点，而是其终点处的安全收口：前序机制负责尽量让请求走对路，拦截器负责在走错路时将其挡在门外。通过这种分层化设计，系统既能保持任务内协同调用路径的效率，又能在多任务并发条件下维护稳定、可验证的任务边界。

综上，基于 TaskId 的入站一致性校验与安全阻断机制，使实例服务入口成为任务边界在数据面的最终执行点。通过将 TaskId 完整性校验、实例生命周期约束和跨任务一致性判定集中收敛于 gRPC 服务端拦截器，本文在“控制面完成任务分配、数据面完成任务内动态寻址”的基础上，进一步补齐了异常场景下的强制隔离能力，从而形成了任务内协同通信从初始化、寻址到入站校验的完整运行期闭环。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=14cm]{5.7.png}
    \caption{直连异常回退路径}
...
\begin{figure}[H]
    ...
    \caption{实例侧入口 Envoy HTTP 过滤器链组成}
...
\begin{figure}[htbp]
    ...
    \caption{TaskManager 资源竞态处理控制台日志}
...
\begin{figure}[htbp]
    ...
    \caption{Envoy 基于 Lua 脚本执行动态寻址与流量分发的过滤日志}
...
\begin{figure}[htbp]
    ...
    \caption{浏览器开发者工具网络面板展示的双通道分流结果}
...
\begin{figure}[htbp]
    ...
    \caption{实例侧 Envoy 基于 slotMap 执行本地校验与转发的观测日志}
...
\begin{figure}[htbp]
    ...
    \caption{Service Worker 捕获异常并执行降级回退的控制台日志}
...
\subsection{多实例并行下的控制流抖动与隔离性验证实验}
...
尽管本文围绕遗留分布式仿真系统云端化过程中的统一接入、任务级状态隔离与双通道传输优化等关键问题进行了设计、实现与验证，但受实验环境与研究范围所限，相关工作仍有进一步拓展空间。后续研究可从以下三个方面展开。
...
第二，可进一步开展复杂网络环境下的双通道传输机制研究。
本文实验主要基于局域网环境展开，重点验证了控制流与态势流分离、实例侧路径校验以及异常回退机制的有效性。对于跨地域部署、广域网传输或多中心协同等更复杂场景而言，网络时延波动、链路带宽受限以及局部网络异常都可能对双通道机制的性能与稳定性产生影响。未来可结合更复杂的网络条件，对双通道路径选择策略、异常恢复效率以及大规模并发条件下的运行表现进行进一步分析与优化，以增强所提方案在实际云端应用环境中的适用性与推广价值。

第三，可进一步研究更高性能的数据面处理机制。
...
\begin{accomplishments}                                     % 参加科研情况开始
    [1]公布发明专利：基于用户态文件与内核虚拟机的异构文件系统集成方法，专利号：202410841033.2。
    [2]公布发明专利：基于eBPF技术和插件化技术的分布式仿真系统设计方法，专利号：202410868995.7。
