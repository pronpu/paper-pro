...
%%-----------------------------------------------------------------------------%
\begin{abstract}                                            % 中文摘要开始随着仿真平台向云端部署并引入云原生机制，遗留仿真系统面临在多租户共享与资源池化环境下的新挑战。在此背景下，系统需在最小侵入既有仿真内核的前提下，扩展接入形态、构建统一入口，以适配云环境的开放生态。然而，云端资源池化使得多任务并发运行成为常态，这直接打破了传统有状态仿真基于单任务独占的隔离假设，极易引发跨任务的状态污染，成为制约系统平滑上云的关键瓶颈。 针对上述问题，本文设计并实现了一个具备有状态隔离能力的云端分布式仿真系统。
